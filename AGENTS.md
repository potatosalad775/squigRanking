...
- **The subset of `ranking-config.js` modernGraphTool reads.** mGT v2.2.0 and up can point at a deployed config with its own `RANKING.CONFIG_URL` and read ranks out of the same sheet, so a device list and a ranking page never disagree. It loads the file as a plain `<script>` and reads only `types[<type>].source.url`, `types[<type>].rowFilter`, the `role: 'rank'` column's `source` and `scale`, the `role: 'brand'` / `role: 'model'` column sources, and `deepLink`. Everything else is ignored, and an unrecognized `configVersion` is not an error there — mGT does not own this file. Renaming or restructuring any of those keys breaks someone else's device list silently, so treat them as public.
