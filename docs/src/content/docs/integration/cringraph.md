---
title: 'CrinGraph'
description: 'Make a CrinGraph fork link its review-score badges to your ranking page.'
---
...
- **Hash-based templates and history.** Changing only the `#` hash on an existing ranking-page tab triggers `hashchange` and scrolls without reload. The ranking page listens for `hashchange` (see [src/main.ts](https://github.com/potatosalad775/squigRanking/blob/main/src/main.ts)), so this works out of the box.
