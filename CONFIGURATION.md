...
The authoritative definition is [`src/types.ts`](src/types.ts), published as `dist/types.d.ts`. With the JSDoc annotation at the top of `ranking-config.js`, your editor validates the whole object as you type.
...
Three ready-made configurations live in [`presets/`](presets/): `letter` (S through F), `stars` (five stars in half steps), and `score` (0 to 10). Each ships its own `TEMPLATE.csv` matching that scale. Copy the pair you want over `ranking-config.js` and `TEMPLATE.csv`.
